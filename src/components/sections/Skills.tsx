...
import {
  ...
  SiHtml5, SiCss, SiRedux, SiFramer, SiNodedotjs, SiExpress,
  SiPostman, SiMongodb, SiMysql, SiFirebase, SiGit, SiGithub,
  SiDocker, SiVercel, SiPython, SiCplusplus, SiLinux, SiNpm,
  SiFigma
} from 'react-icons/si';
...
const iconMap: Record<string, React.ComponentType<{ size?: number; color?: string; style?: React.CSSProperties }>> = {
  ...
  SiPostman, SiMongodb, SiMysql, SiFirebase, SiGit, SiGithub,
  SiDocker, SiVercel, SiPython, SiCplusplus, SiLinux, SiNpm,
  SiFigma, SiVisualstudiocode: VscVscode,
};

const categoryTabs: { key: SkillCategory | 'all'; label: string; color: string }[] = [
...
const SkillCard: React.FC<{ skill: typeof skills[0]; index: number; isInView: boolean }> = ({ skill, index, isInView }) => {
  ...
  return (
    <motion.div
      ...
      style={hovered ? { borderColor: `${skill.color}30`, boxShadow: `0 0 20px ${skill.color}15, 0 4px 20px rgba(0,0,0,0.3)` } : {}}
    >
      {/* Icon */}
      <div className="flex items-center gap-3 mb-3">
        ...
        <span className="font-semibold text-white/90 text-sm font-display">{skill.name}</span>
      </div>
      ...
      <div className="skill-bar">
        <motion.div
          ...
          transition={{ duration: 1.2, delay: index * 0.04 + 0.4, ease: [0.22, 0.61, 0.36, 1] }}
          style={{ background: `linear-gradient(90deg, ${skill.color}99, ${skill.color})` }}
        />
      </div>

      <div className="flex justify-between items-center mt-1.5">
        <span className="text-xs text-white/25 capitalize">{skill.category}</span>
...
const Skills: React.FC = () => {
  ...
  return (
    <section id="skills" className="relative py-24 overflow-hidden" aria-label="Skills section">
      {/* Background */}
      ...
      <div className="section-container" ref={ref}>
        ...
        >
          <div className="flex items-center gap-3 mb-4">
            ...
            <span className="text-purple-400 text-sm font-mono tracking-wider uppercase">Technical Expertise</span>
          </div>
          ...
          <p className="section-subtitle mt-3">
            A curated toolkit of modern technologies I use to build production-ready applications.
          </p>
        </motion.div>
        ...
        <AnimatePresence mode="wait">
          ...
          >
            {filtered.map((skill, i) => (
              <SkillCard key={`${skill.name}-${skill.category}`} skill={skill} index={i} isInView={isInView} />
            ))}
          </motion.div>
        </AnimatePresence>
      </div>
